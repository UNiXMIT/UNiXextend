...
:: SET ACUDIR32="C:\Program Files (x86)\Micro Focus\extend %EXTEND%\"
:: SET ACUDIR64="C:\Program Files\Micro Focus\extend %EXTEND%\"
:: SET SAMPLEDIR="C:\Users\Public\Documents\Micro Focus\extend %EXTEND%\"
SET ACUBIT=
SET CFLAGS=FALSE
SET ACUPATCH=
SET ACUJAVA=
...
:SETACUENV
IF '%ACUDIR32%'=='' FOR /F "tokens=2*" %%A IN ('REG.EXE QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Micro Focus\ACUCOBOL-GT\%EXTEND%" /S /V "INSTALLDIR" 2^>NUL ^| FIND "REG_SZ"') DO SET "ACUDIR32=%%B"
IF '%ACUDIR64%'=='' FOR /F "tokens=2*" %%A IN ('REG.EXE QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Micro Focus\ACUCOBOL-GT\%EXTEND%" /S /V "INSTALLDIR" 2^>NUL ^| FIND "REG_SZ"') DO SET "ACUDIR64=%%B"
IF '%SAMPLEDIR%'=='' FOR /F "tokens=2*" %%A IN ('REG.EXE QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Micro Focus\ACUCOBOL-GT\%EXTEND%" /S /V "PUBLICDIR" 2^>NUL ^| FIND "REG_SZ"') DO SET "SAMPLEDIR=%%B"
SET DEF=".;%SAMPLEDIR%sample\def"
SET XML=".;%SAMPLEDIR%sample\xmlext"
SET BMP=".;%SAMPLEDIR%sample\acubench\resource"
SET ALL=".;%SAMPLEDIR%sample\def;%SAMPLEDIR%sample\xmlext;%SAMPLEDIR%sample\acubench\resource"
SET COPYPATH=%ALL%
...
:ATW
IF '%ACUBIT%'=='32' (
    START "" "%ACUDIR32%AcuGT\bin%ACUPATCH%\AcuToWeb.exe"
)
IF '%ACUBIT%'=='64' (
    START "" "%ACUDIR64%AcuGT\bin%ACUPATCH%\AcuToWeb.exe"
)
GOTO :END

:ACUBENCH
IF '%ACUBIT%'=='32' (
    START "" "%ACUDIR32%acubench\AcuBench.exe"
)
IF '%ACUBIT%'=='64' (
    START "" "%ACUDIR64%acubench\AcuBench.exe"
)
GOTO :END
...
:32BIT
SET ACUBENCH=%ACUDIR32%acubench\
SET PATH=C:\AcuScripts;%ACUDIR32%AcuGT\bin%ACUPATCH%;%ACUBENCH%;C:\WINDOWS\system32
SET GENESIS_HOME=%ACUDIR32%AcuGT
GOTO :SETEXTRAS

:64BIT
SET ACUBENCH=%ACUDIR64%acubench\
SET PATH=C:\AcuScripts;%ACUDIR64%AcuGT\bin%ACUPATCH%;%ACUBENCH%;%ACUDIR32%AcuGT\bin%ACUPATCH%;C:\WINDOWS\system32
SET GENESIS_HOME=%ACUDIR64%AcuGT
GOTO :SETEXTRAS
...
SET CLASSPATH=.;%ACUDIR32%AcuGT\bin%ACUPATCH%\CVM.jar;%ACUDIR32%AcuGT\bin%ACUPATCH%\vortex.jar
...
SET PATH=%JAVA_HOME%\bin\client;%JAVA_HOME%\bin;%PATH%
SET CLASSPATH=.;%ACUDIR64%\AcuGT\bin%ACUPATCH%\CVM.jar;%ACUDIR64%AcuGT\bin%ACUPATCH%\vortex.jar
SET PRELOAD_JAVA_LIBRARY=1
SET JAVA_LIBRARY_NAME=%JAVA_HOME%\bin\client\JVM.dll
GOTO :SETEXTRAS
