...
:SETACUENV
IF '%INSTALLDIR32%'=='' FOR /F "tokens=2*" %%A IN ('REG.EXE QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Micro Focus\ACUCOBOL-GT\%EXTEND%" /S /V "INSTALLDIR" 2^>NUL ^| FIND "REG_SZ"') DO SET "INSTALLDIR32=%%B"
IF '%INSTALLDIR64%'=='' FOR /F "tokens=2*" %%A IN ('REG.EXE QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Micro Focus\ACUCOBOL-GT\%EXTEND%" /S /V "INSTALLDIR" 2^>NUL ^| FIND "REG_SZ"') DO SET "INSTALLDIR64=%%B"
IF '%DEFPATH32%'=='' FOR /F "tokens=2*" %%A IN ('REG.EXE QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Micro Focus\ACUCOBOL-GT\%EXTEND%" /S /V "PUBLICDIR" 2^>NUL ^| FIND "REG_SZ"') DO SET "DEFPATH32=%%B"
IF '%DEFPATH64%'=='' FOR /F "tokens=2*" %%A IN ('REG.EXE QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Micro Focus\ACUCOBOL-GT\%EXTEND%" /S /V "PUBLICDIR" 2^>NUL ^| FIND "REG_SZ"') DO SET "DEFPATH64=%%B"
SET DEF=".;%DEFPATH32%sample\def"
SET XML=".;%DEFPATH32%sample\xmlext"
SET BMP=".;%DEFPATH32%sample\acubench\resource"
SET ALL=".;%DEFPATH32%sample\def;%DEFPATH32%sample\xmlext;%DEFPATH32%sample\acubench\resource"
SET COPYPATH=%ALL%
