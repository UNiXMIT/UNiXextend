       ...
       PROGRAM-ID.      520_1793.
       author.          marie
       date-written.    11-Apr-01
       remarks.
	   compile_options_required: -D5
	   runtime_options_required: -c Config  
	   original_version_tested: 520 alpha
	   purpose: The MQSeries compatibility interface was not correct.
		    It would not pass the values from the COBOL program
		    into the MQSeries library correctly.  The result is the
		    inability to specify a buffer size larger than 127 bytes
		    for putting and getting messages from the queue.  Invalid
		    buffer size error message is returned from the call to 'MQPUT'.
		    QA has to test the various MQSeries calls to ensure that
		    the fix did not adversely affect anything.
      ...
      *    Indicate that sample program has started
	   ...
	   MOVE 125 TO Max-Buff-Length.
	   DISPLAY "Buffer Length may be from 126 to 1024 bytes." 
	     Line 6, Col 5.
	   perform connect-to-queue-manager.
	   accept omitted.
	   if COMPCODE = 0           
	       perform main-loop until max-buff-length = (126 + 4)
	   end-if.
	   display "Tests Complete!  520_1793 passed.".
      ...
      -            "
      -            "                    " line 1, col 1.
	   display "Currently testing " Line 1, col 5.
	   ACCEPT Max-Buff-Length  UPDATE col 23.
