...
if command -v yum >/dev/null; then
  ...
  sudo yum install -y libnsl podman unixODBC wget curl cronie dos2unix java-11-openjdk htop tmux libstdc++.i686 libxcrypt.i686 ncurses-libs-6.1-9.20180224.el8.i686 libaio-devel glibc.i686 zlib-1.2.11-18.el8_5.i686;
  sudo setenforce 0;
  ...
  sudo apt install -y build-essential podman unixodbc-dev wget curl cron dos2unix default-jdk htop tmux lib32stdc++6 libaio-dev libncurses5 apt-file zlib1g:i386;
  sudo apt-file update;
elif command -v zypper >/dev/null; then
  sudo zypper refresh;
  sudo zypper update -y;
  sudo zypper install -y -t pattern devel_C_C++;
  sudo zypper install -y cronie podman unixODBC wget curl dos2unix java-11-openjdk htop tmux libncurses5-32bit libaio-devel libstdc++6-32bit libgcc_s1-32bit libz1-32bit;
else
